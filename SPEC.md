...
- **Editor layout (Implemented).** The editing surface is a **persistent top banner** over a **two-zone body** (a drawer sidebar + the work area), replacing the old fixed three-column grid that squeezed unusably on small screens. The banner is the single home for cross-cutting status and actions: brand, repo ref, the aggregate change/save-status, the Content⇄Advanced toggle, the preview-layout control, and the **Publish** button — so the sidebar carries only navigation (the object list + ＋New) and is free to hide. **Preview has three layouts:** **Split** (side-by-side, with a **draggable divider** whose width is clamped and persisted), **Tabs** (one pane at a time via an Edit⇄Preview switch), and **Hide** (editor full-width); a **pop-out** button opens the live preview — the same real-generator render — in its own window (a truer full-page view, and the mirror updates as you edit). The **sidebar is a toggleable drawer** (a banner button, plus ⌘/Ctrl-B); on narrow viewports it becomes an overlay over a scrim. **Responsive defaults** key off a single ~900px breakpoint: desktop opens the sidebar and defaults to Split; mobile hides the sidebar, drops Split from the choices, and defaults to Tabs, with the per-page editor-header actions (Discard/Rename/Delete) folding into a **⋯ overflow menu**. Layout choices are **device-local UI preferences persisted in `localStorage`** (sidebar open/closed, preview mode, split width) — distinct from the token, which stays out of `localStorage` (§9); a stored preference always overrides the viewport-derived default.
